defmodule Portico.Audit do
  @moduledoc "Synchronous audit hook invoked before scanner launches."

  @type event :: :scanner_launch
  ...
  def emit(hook, event, metadata) when is_function(hook, 2) do
    case hook.(event, metadata) do
      :ok -> :ok
      other -> raise "audit hook returned #{inspect(other)}, expected :ok"
    end
  end
